...
def test_add_and_get_rule(mem_db):
    ...
    assert retrieved_rule["trigger"] == rule_data["trigger"]
...
def test_save_new_device_with_config(mem_db):
    """
    Testuje zapis nowego urządzenia.
    """
    ...
    assert device["state"] == '{"state": "ON", "brightness": 150}'

def test_update_existing_device_state_only(mem_db):
    """
    Testuje aktualizację TYLKO STANU istniejącego urządzenia.
    """
